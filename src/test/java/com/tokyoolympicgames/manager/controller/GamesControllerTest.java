...
public class GamesControllerTest {
    ...
    public void setUp() {
        ...
        game2 = new Game(LocalDateTime.now()
                                      .plusDays(1), LocalDateTime.now()
                                                                 .plusDays(1)
                                                                 .plusHours(2), "Basketball", "Spain", "USA", "Tokyo",
            Stage.FINAL);
    }
    ...
    public void testPersistGameCorrectly() throws Exception {
        ...
        Mockito.when(service.findByModalityAndLocal("Soccer", "Tokyo"))
               .thenReturn(new ArrayList<>());
        Mockito.when(service.findByLocal("Tokyo"))
    ...
    public void testPersistGameWithNullOrEmptyFields() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.BAD_REQUEST, controller.persistGame(game)
                                                              .getStatusCode());
        Assert.assertEquals(
            Arrays.asList("Invalid: [modality can not be null or empty, local can not be null or empty]"),
    ...
    public void testPersistGameWhereStageIsNotFinalNorSemiFinalWithSameCountry() throws Exception {
        ...
        game.setFirstCountry("Brazil");
        game.setSecondCountry("Brazil");
        Mockito.when(service.findByModalityAndLocal("Soccer", "Tokyo"))
               .thenReturn(new ArrayList<>());
        Mockito.when(service.findByLocal("Tokyo"))
    ...
    public void testGetGameWithFilter() {
        ...
        Mockito.when(service.findByModalityOrderByBeginTime("Soccer"))
               .thenReturn(games);
